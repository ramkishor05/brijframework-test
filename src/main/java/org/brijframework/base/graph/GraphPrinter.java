package org.brijframework.base.graph;
...
public class GraphPrinter {
	...
	public boolean validateString(String aString) {
		for (int i = 0; i < aString.length(); i++) {
			...
			if (((c < 65) || (c > 90)) && c != 32 && c != 40 && c != 41) {
				LogTracker.info("ObjectPrinter_validateString",LogAccess.DEVELOPER, "Not valid content in indexof " + i + " : " + c);
				return false;
			}
		}
		return true;
	...
	public Object printToFile(File file) {
		if (!file.exists()) {
              LogTracker.info("ObjectPrinter_printToFile",LogAccess.DEVELOPER, "file not found");
              return null;
		}
		this.printObject();
		try {
			...
			LogTracker.trace("ObjectPrinter_printToFile",LogAccess.DEVELOPER,file, e,"IO file problem");
		}
		return stringToCopy.toString();
	}
